...
/**
 * Data source for MAP columns with columnar map index. Wraps a {@link ColumnarMapIndexReader} and exposes it for
 * query access.
 *
...
public class ColumnarMapDataSource extends BaseDataSource implements MapDataSource {
  ...
  /**
   * MAP columns with columnar map index use per-key typed storage rather than the homogeneous
  ...
  public DataSourceMetadata getKeyDataSourceMetadata(String key) {
    throw new UnsupportedOperationException("getKeyDataSourceMetadata not supported for MAP with columnar map index");
  }

  @Override
  public ColumnIndexContainer getKeyIndexContainer(String key) {
    throw new UnsupportedOperationException("getKeyIndexContainer not supported for MAP with columnar map index");
  }

  private DataSource buildKeyDataSource(String key) {
    FieldSpec.DataType keyType = _columnarMapIndexReader.getKeyValueType(key);

    // Check if this is a sparse key (type is known from CMAP metadata but no per-key data)
    if (keyType == null) {
      return null;
    }
    ...
    if (_columnarMapIndexReader instanceof ImmutableColumnarMapIndexReader) {
      ...
      if (tierFlag == ImmutableColumnarMapIndexReader.TIER_SPARSE) {
        // Sparse key: build DataSource backed by sparse-section JSON blob reader
  ...
  /**
   * Builds a DataSource for a sparse-tier key backed by the sparse-section JSON blob reader.
  ...
  private DataSource buildSparseKeyDataSource(String key, FieldSpec.DataType keyType,
    ...
    // Create a ForwardIndexReader that reads from the in-CMAP sparse JSON section
    ...
    ForwardIndexReader<ForwardIndexReaderContext> sparseReader = new ForwardIndexReader<ForwardIndexReaderContext>() {
      ...
      public String getString(int docId, ForwardIndexReaderContext context) {
        ...
        try {
          ...
          throw new RuntimeException("Failed to parse sparse-section JSON for column='" + getColumnName()

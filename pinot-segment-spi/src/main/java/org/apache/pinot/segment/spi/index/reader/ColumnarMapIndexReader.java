 ...
 *
 * <p>Implementations must be safe for concurrent reads. Mutable implementations may impose
 * a single-writer constraint; refer to the concrete implementation's Javadoc for details.
 *
 * <p>Per-key {@code DataSource} construction is the responsibility of the surrounding
 * {@code ColumnarMapDataSource} wrappers, not this reader. This interface exposes only
...
public interface ColumnarMapIndexReader extends IndexReader {
   ...
   * non-null value at {@code docId} appear in the result. Returns an empty map if the
   * document has no values; behavior for an out-of-range {@code docId} is implementation-defined.
   */
